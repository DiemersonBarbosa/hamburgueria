...
function updateCartModal(){
    ...
    cart.forEach(item => {
        ...
        <div class="flex items-center justify-between max-h-[30%]">
            ...
            <div>
                <button class="remove-from-cart-btn bg-red-500 rounded px-2 text-bold" data-name="${item.name}">
                    X
                </button>

                <button onclick="addQuantity()"  class=" bg-green-500 rounded px-2 text-bold">
                    +
                </button>
...
const cartItems = cart.map((item) => {
    return(
       `*✅${item.name}* \n*Quantidade:* (_${item.quantity}_) \n*Preço:* R$ _${item.price.toFixed(2)}_ \n\n`
    )
}).join("")

const message = encodeURIComponent(cartItems)
const phone = "41998406295"

window.open(`https://wa.me/${phone}?text=📝 *_MEU PEDIDO_*%0A%0A${message}
